...
int assign(prog_data* data)
{
    ...
    if (IS_VALUE(data->token)) {
        ...
        //<assign> -> id( <term> )
        //ID is a function
        if (symtable_search_function(data->global_table, data->token.attribute, *tmp)) {
        ...
        if (data->local_table == NULL) {
            if (symtable_search_variable(data->global_table, data->token.attribute, *tmp)) {
        ...
        //we in function
        if (data->local_table != NULL){
            if (symtable_search_variable(data->local_table, data->token.attribute, *tmp)) {
